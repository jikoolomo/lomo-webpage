...
현재 소스에는 고유한 페이지 제목과 한국어 메타 설명, 방문자 언어에 따른 `lang`, `robots` 메타 태그, Open Graph·Twitter 카드, 작성자 정보, `WebSite`/`Person` JSON-LD, `robots.txt`, `sitemap.xml`, canonical URL, GitHub Pages 절대 공유 이미지, 히어로 이미지의 설명형 대체 텍스트가 포함되어 있습니다. Google은 검색 결과의 제목을 `<title>`, 주요 시각 제목, headings, `og:title`, 링크 텍스트, `WebSite` 구조화 데이터 등 여러 신호를 조합해 자동 생성하므로, 이 요소들이 같은 브랜드·페이지 주제를 가리키도록 맞추는 것이 중요합니다. [1]
...
| 설정 | 추가 위치 | 해야 할 일 |
|---|---|---|
| Canonical URL | `index.html`의 `<head>` | `https://jikoolomo.github.io/lomo-webpage/` 적용 완료 |
| Sitemap | 루트의 `sitemap.xml` | `https://jikoolomo.github.io/lomo-webpage/` 적용 완료 및 `robots.txt` 연결 |
...
GitHub Pages 공개 주소는 `https://jikoolomo.github.io/lomo-webpage/`입니다. Google Search Console과 Bing Webmaster Tools에서 해당 주소의 소유권을 확인하고 sitemap URL을 제출하면 크롤링·색인 상태를 확인할 수 있습니다. Google은 변경 사항이 검색 결과에 반영되기까지 시간이 걸릴 수 있다고 안내하므로, 즉시 노출이나 순위 상승을 보장할 수는 없습니다. [1]
